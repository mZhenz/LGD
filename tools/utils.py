...
def save_checkpoint_mod(state, is_best, filename='checkpoint.pth.tar', prefile='checkpoint.pth.tar'):
    torch.save(state, filename)
...
def adjust_learning_rate_generator(optimizer, epoch, args):
    """Decay the learning rate based on schedule"""
    lr = args.lr_G * args.lr_mult
    if epoch < args.warmup_epochs:
        # warm up
        lr = args.lr_G + (args.lr_G * args.lr_mult - args.lr_G) / args.warmup_epochs * epoch
    elif args.cos:  # cosine lr schedule
        lr *= 0.5 * (1. + math.cos(math.pi * epoch / args.epochs))
    else:  # stepwise lr schedule
        for milestone in args.schedule:
...
def mcs_soft_cross_entropy(student_logit, teacher_logit, mcs):
    '''
    mean classification score guided loss
    :param student_logit: logit of the student arch (without softmax norm)
    :param teacher_logit: logit of the teacher arch (already softmax norm)
    :return: CE loss value.
    '''
    # print(student_logit.shape, teacher_logit.shape, mcs.shape)
    return -(teacher_logit * torch.nn.functional.log_softmax(student_logit, 1) * mcs).sum()/student_logit.shape[0]
